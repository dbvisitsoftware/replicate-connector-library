 ...
 */
public class ChangeRowParserTest extends ChangeParserTestConfig {
    private static final Logger logger = LoggerFactory.getLogger(
        ChangeRowParserTest.class
    );
    ...
    public void testParseTimestampTzLCR() {
        ...
        */
        ...
        try {
            ...
            logger.info (
                "DTZ: " + dtz.toString() + " " + 
                "UTC: " + dateFormatUTC.format(dtz)
            );
            
            logger.info ("Date from UTC: " + d.toString());
            
            logger.info (
                "DTZ epoch time: " + dtz.getTime() + " tz offset: " +
                dtz.getTimezoneOffset() + " " +
                "Date from UTC epoch time: " + d.getTime() + " tz offset: " +
                d.getTimezoneOffset() 
            );
    ...
    public void testParseInsertLCR(
        ...
        try {
            ...
            ChangeRowParser lp = new ChangeRowParser();
            
            for (int i = 0; i < LCR_INSERT_NUM_ENTRY_RECORDS; i++) {
                lp.parse(plog, records.get(i));
            
                ChangeRowRecord lcr = (ChangeRowRecord)lp.emit();
                ...
                if (lcr.getAction().equals (ChangeAction.INSERT)) {
                    ...
                    */
                    ...
                    if (cr.getValue() instanceof Timestamp) {
                        Timestamp t1 = ((Timestamp)cr.getValue());
                        Timestamp t2 = ((Timestamp)columnValue);
                        logger.info (
                            "Timestamp decoded: " + t1.getTime() + " " + 
                            "tzh: " + t1.getTimezoneOffset() + " " +
                            "expecting: " + t2.getTime() + " " +
                            "tzh: " + t2.getTimezoneOffset()
                        );
                    }
